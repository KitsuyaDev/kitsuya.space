...
const App: React.FC = () => {
  ...
  const togglePerformance = () => {
    ...
    // Force clear particles when switching to performance mode
    if (!performanceMode) setParticles([]);
    playSound('click');
  };
    ...
    <div id="root-container" className={`max-w-[1400px] mx-auto space-y-6 md:space-y-8 relative z-10 py-4 px-3 md:px-6 transition-all duration-1000 ${isPanic ? 'bg-red-950/40' : ''}`}>
        ...
        {showSettings && (
          <div className="dimden-panel p-4 w-64 space-y-4 animate-in fade-in slide-in-from-bottom-6 duration-500 shadow-2xl">
            ...
            <button 
              ...
              className={`w-full flex items-center justify-between p-3 rounded terminal-font text-lg transition-all ${!performanceMode ? 'bg-pink-500/15 text-pink-200' : 'hover:bg-black/5 dark:hover:bg-white/5 text-black/40 dark:text-white/40'}`}
            >
              <div className="flex items-center gap-3">
                <CpuIcon size={16} />
                <span>FX Rendering</span>
              </div>
              <div className={`w-8 h-4 rounded-full p-0.5 border border-white/20 transition-all ${!performanceMode ? 'bg-pink-500' : 'bg-black/10 dark:bg-white/10'}`}>
            ...
            <button 
              ...
              className={`w-full flex items-center justify-between p-3 rounded terminal-font text-lg transition-all ${lightMode ? 'bg-pink-500/15 text-pink-600' : 'hover:bg-black/5 dark:hover:bg-white/5 text-black/40 dark:text-white/40'}`}
            >
              <div className="flex items-center gap-3">
                {lightMode ? <Sun size={16} /> : <Moon size={16} />}
                <span>Luminance <span className="text-[10px] opacity-50 font-bold">[BETA]</span></span>
              </div>
              <div className={`w-8 h-4 rounded-full p-0.5 border border-white/20 transition-all ${lightMode ? 'bg-pink-500' : 'bg-black/10 dark:bg-white/10'}`}>
      ...
      <header className={`dimden-panel p-0 overflow-hidden group border-pink-400/10 hover:border-pink-400/30 ${rainbowMode ? 'shadow-[0_0_40px_rgba(255,255,255,0.2)]' : ''}`}>
        <div className="bg-black/[0.03] dark:bg-white/[0.03] p-2 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
          <h3 className="pixel-title text-[7px] opacity-40 uppercase tracking-[0.3em] flex items-center gap-2 drop-shadow-sm">
        ...
        {!performanceMode && <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-[0.05] z-0">
        ...
        <div className="p-4 sm:p-6 md:p-10 flex flex-col md:flex-row items-center justify-between relative overflow-hidden gap-6">
          <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-10 relative z-10 text-center sm:text-left">
            ...
            <div className="cursor-pointer select-none" onClick={handleHeaderClick}>
              <h1 className="pixel-title text-2xl sm:text-3xl md:text-4xl mb-3 transition-all tracking-tight uppercase flex items-center justify-center sm:justify-start gap-4 drop-shadow-md">
              ...
              <div className="flex items-center justify-center sm:justify-start gap-4">
                ...
                <p className={`terminal-font text-xl sm:text-2xl opacity-70 uppercase tracking-widest drop-shadow-sm ${lightMode ? 'text-pink-700' : 'text-pink-100'}`}>
          ...
          <div className="flex gap-10 sm:gap-16 md:border-l border-black/5 dark:border-white/5 md:pl-12 lg:pl-16 relative z-10">
             <div className="flex flex-col items-center">
               <span className="text-black/30 dark:text-white/20 font-bold uppercase tracking-[0.4em] mb-2 text-[8px] drop-shadow-sm">ALLOC_RAM</span>
               <span className={`terminal-font text-2xl sm:text-3xl transition-all duration-700 drop-shadow-md ${isPanic ? 'text-red-600' : 'text-pink-900 dark:text-pink-100'}`}>
                {liveMem.toFixed(2)}GB <span className="text-black/20 dark:text-white/10 text-sm">/ 28GB</span>
               </span>
             </div>
             <div className="flex flex-col items-center">
               <span className="text-black/30 dark:text-white/20 font-bold uppercase tracking-[0.4em] mb-2 text-[8px] drop-shadow-sm">SYNC_TPS</span>
               <span className={`terminal-font text-4xl sm:text-5xl drop-shadow-[0_4px_10px_rgba(74,222,128,0.2)] ${isPanic ? 'text-red-600' : 'text-green-600 dark:text-green-400'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
        <aside className="md:col-span-3 space-y-8 order-2 md:order-1">
          ...
          <div className="dimden-panel p-0 overflow-hidden group border-black/5 dark:border-white/5">
            <div className="bg-black/[0.02] dark:bg-white/[0.02] p-3 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
              <h3 className="pixel-title text-[7px] opacity-40 uppercase tracking-widest drop-shadow-sm">Profile</h3>
            ...
            <div className="p-5 space-y-2">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-center py-2 border-b border-black/[0.03] dark:border-white/[0.03] last:border-0 hover:bg-pink-500/5 transition-all px-2 rounded">
                    <span className="terminal-font text-black/40 dark:text-white/30 text-base uppercase tracking-widest drop-shadow-sm">{item.label}</span>
                    <span className="terminal-font text-xl text-pink-900 dark:text-pink-100 drop-shadow-sm">{isPanic ? '???' : item.value}</span>
                  </div>
                ))}
            </div>
          </div>
          ...
          <div className="dimden-panel p-0 overflow-hidden group border-black/5 dark:border-white/5">
            <div className="bg-black/[0.02] dark:bg-white/[0.02] p-3 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
              <h3 className="pixel-title text-[7px] opacity-40 uppercase tracking-widest drop-shadow-sm">Links</h3>
            ...
            <nav className="flex flex-col p-2 gap-1">
              {[
                ...
                <a key={idx} href={link.href} target="_blank" className={`sidebar-link group/link !text-xl !py-3 px-4 rounded-lg flex items-center gap-4 drop-shadow-sm ${lightMode ? 'text-pink-800 hover:text-pink-600' : 'text-white'}`} onClick={handleLinkClick} onMouseEnter={() => playSound('hover')}>
          ...
          <div className="dimden-panel p-0 overflow-hidden group border-black/5 dark:border-white/5">
             <div className="bg-black/[0.02] dark:bg-white/[0.02] p-3 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
               <h3 className="pixel-title text-[7px] opacity-40 uppercase tracking-widest flex items-center gap-2 drop-shadow-sm">
                 {track?.nowPlaying ? (
               ...
               {track?.nowPlaying ? (
                 <span className="flex items-center gap-1.5">
                   ...
                   <span className={`w-2 h-2 bg-green-500 rounded-full ${!performanceMode ? 'animate-pulse shadow-[0_0_10px_rgba(74,222,128,0.5)]' : ''}`} />
                 </span>
               ) : (
                 <span className="flex items-center gap-1.5">
             ...
             <div className="p-4">
               {track ? (
                 <a href={track.url} target="_blank" className="flex items-center gap-4 group/track drop-shadow-sm">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="terminal-font text-lg leading-tight truncate text-black dark:text-white font-bold">{track.name}</p>
        ...
        <main className="md:col-span-6 space-y-8 order-1 md:order-2">
          ...
          <section className="dimden-panel p-0 overflow-hidden group border-black/5 dark:border-white/5">
            <div className="bg-black/[0.02] dark:bg-white/[0.02] p-4 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
              <h3 className="pixel-title text-[8px] opacity-40 uppercase tracking-[0.3em] flex items-center gap-2 drop-shadow-sm">
            ...
            <div className="p-5 sm:p-6 relative overflow-hidden flex flex-col justify-start">
              {!performanceMode && <div className="absolute -bottom-2 -right-2 opacity-[0.03] pointer-events-none group-hover:opacity-[0.06] transition-opacity duration-1000">
                <Terminal size={120} className="text-black dark:text-white" />
              </div>}
              <div className={`terminal-font text-xl sm:text-2xl space-y-3 relative z-10 drop-shadow-sm ${lightMode ? 'text-black font-medium' : 'text-pink-50'}`}>
                {isPanic ? (
                  ...
                  <>
                    <p className="text-3xl sm:text-4xl text-pink-700 dark:text-pink-400 font-bold tracking-tight drop-shadow-md">hihi :3 im kit</p>
          ...
          <section className="dimden-panel p-0 overflow-hidden group/hosting border-pink-500/20 hover:border-pink-500/40 relative">
            <div className="bg-pink-500/[0.04] p-4 border-b border-pink-500/10 flex items-center justify-between">
               <h2 className="pixel-title text-[8px] uppercase tracking-[0.4em] text-pink-700 dark:text-pink-300/40 flex items-center gap-2 drop-shadow-sm">
            ...
            <div className="p-8">
              <div className="flex flex-col sm:flex-row items-center gap-8 mb-8">
                ...
                <div className="flex-1 text-center sm:text-left drop-shadow-sm">
                  <h3 className="terminal-font text-4xl uppercase tracking-[0.2em] mb-2 text-black dark:text-white font-bold">Pyro</h3>
              ...
              <div className="dimden-panel border-pink-500/20 bg-pink-500/[0.05] p-6 mb-8 relative rounded-xl backdrop-blur-3xl shadow-lg">
                <div className="flex gap-5 items-start">
                   <ThumbsUp size={28} className="text-pink-700 dark:text-pink-400 shrink-0 mt-1 opacity-70" />
                   <p className="terminal-font text-2xl italic leading-relaxed text-pink-950 dark:text-white/90 drop-shadow-sm">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-8">
                <a href="https://pyro.host/?a=41" target="_blank" className="w-full sm:w-auto inline-flex dimden-panel px-16 py-4 items-center justify-center gap-4 terminal-font text-3xl text-white bg-pink-600 dark:bg-pink-500/10 border-pink-500/40 hover:bg-pink-700 dark:hover:bg-pink-500/20 hover:shadow-[0_0_40px_rgba(255,183,197,0.2)] transition-all group/btn rounded-lg shadow-xl" onClick={handleLinkClick}>
                  <span className="drop-shadow-sm">Visit</span>
                  <ExternalLink size={20} className="group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                </a>
                
                <div className="flex items-center gap-4 text-black/40 dark:text-white/20 terminal-font text-base uppercase tracking-[0.2em] border-l border-black/10 dark:border-white/5 pl-8 h-10 drop-shadow-sm">
                  <Shield size={20} className="opacity-40" />
                  <span>Trust_Verified</span>
                </div>
              </div>

              <div className="mt-8 p-4 border-t border-black/5 dark:border-white/5 flex items-start gap-5 text-black/50 dark:text-white/30 terminal-font text-sm uppercase tracking-widest leading-relaxed drop-shadow-sm">
            ...
            {!performanceMode && <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-[0.02]">
        ...
        <aside className="md:col-span-3 space-y-8 order-3">
          <div className="dimden-panel p-0 overflow-hidden group border-black/5 dark:border-white/5">
             <div className="bg-black/[0.02] dark:bg-white/[0.02] p-3 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
               <h3 className="pixel-title text-[7px] opacity-40 uppercase tracking-widest drop-shadow-sm">Heartbeat</h3>
               <Activity size={14} className="text-black/20 dark:text-white/10" />
             </div>
             <div className="p-5 terminal-font space-y-6">
                <div className="flex justify-between items-end border-b border-black/5 dark:border-white/5 pb-3 drop-shadow-sm">
                  <span className="text-black/40 dark:text-white/20 text-sm tracking-[0.2em] uppercase">UPTIME</span> 
                  <span className={`text-2xl font-bold ${isPanic ? 'text-red-600' : 'text-pink-900 dark:text-pink-100'}`}>99.9%</span>
                </div>
                <div className="flex gap-1.5 h-10 items-end">
                   {[40, 70, 30, 90, 50, 80, 20, 60, 45, 75, 55, 85].map((h, i) => (
                     <div key={i} className={`w-full transition-all duration-1000 rounded-t-sm bg-pink-500/20 dark:bg-pink-500/10 group-hover:bg-pink-500/40 ${performanceMode ? 'duration-0' : ''}`} style={{height: `${h}%`}} />
                   ))}
                </div>
             </div>
          </div>
          ...
          <div className="dimden-panel p-0 overflow-hidden group border-black/5 dark:border-white/5">
             <div className="bg-black/[0.02] dark:bg-white/[0.02] p-3 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
               <h3 className="pixel-title text-[7px] opacity-40 uppercase tracking-widest drop-shadow-sm">Hardware</h3>
             ...
             <div className="p-5 terminal-font space-y-4 relative z-10">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-center py-3 border-b border-black/[0.03] dark:border-white/[0.03] last:border-0 hover:bg-pink-500/5 px-3 rounded-lg transition-all group/item border-l-2 border-transparent hover:border-pink-500/30">
                    <span className="text-black/50 dark:text-white/40 uppercase text-sm tracking-[0.2em] font-bold drop-shadow-sm">{spec.label}</span>
                    <span className="text-right text-2xl text-pink-900 dark:text-pink-100 group-hover/item:text-pink-700 dark:group-hover/item:text-pink-300 transition-colors font-bold drop-shadow-md">{spec.value}</span>
                  </div>
                ))}
             </div>
          </div>
          ...
          <div className="dimden-panel p-0 overflow-hidden group border-black/5 dark:border-white/5">
             <div className="bg-black/[0.02] dark:bg-white/[0.02] p-3 border-b border-black/5 dark:border-white/5 flex items-center justify-between">
               <h3 className="pixel-title text-[7px] opacity-40 uppercase tracking-widest drop-shadow-sm">Shop</h3>
             ...
             <div className="p-8 text-center drop-shadow-sm">
                ...
                <div className={`text-[10px] terminal-font opacity-20 uppercase tracking-[0.3em] mt-2 ${!performanceMode ? 'animate-pulse' : ''}`}>Waiting for inventory</div>
             </div>
          </div>
        </aside>
      </div>

      <footer className="py-20 text-center terminal-font text-black/5 dark:text-white/5 text-2xl tracking-[0.6em] uppercase hover:text-black/20 dark:hover:text-white/20 transition-all duration-1000 drop-shadow-sm">
